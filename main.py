...
font_general = 62
dado_pos_inicial = Window.size[0]/2
...
class Jugadores:
    ...
    def ver_posibilidades(self):
        ...
        dict1.update(dict2)
        self.diccionarioRes = {k: v for k, v in sorted(dict1.items(), key=lambda item: item[1],reverse=True)}
    ...
    def check_ganador(self):
        ...
        for jugador in self.jugadores_todos:
            self.juegos_completos[jugador] = self.jugadores_todos[jugador].check_estado_juego()[0]
            self.jugadores_ganadores[jugador] = self.jugadores_todos[jugador].check_estado_juego()[1]
            self.puntajes_todos_jugadores[jugador] = self.jugadores_todos[jugador].check_estado_juego()[2]
        ...
        self.puntajes_ordenados = {k: v for k, v in sorted(self.puntajes_todos_jugadores.items(), key=lambda item: item[1],reverse=True)}
...
class GanadorScreen(Screen):
    ...
    def on_pre_enter(self):
        ...
        self.ganador.text = f'¡Felicidades, {self.nombre_jugador}! ¡Ganaste! :D'
...
class LanzadaScreen(Screen):
    ...
    def mostrar_dados(self, _):
        ...
        for i in self.Lanzada:
            dado_inicial = dado_pos_inicial
            pos_x = dado_inicial + random.randint(-220, 220)
            pos_y = dado_inicial + random.randint(-220, 220)
            image = Image(source=f'sprites/dado_{i}.png')
            self.dado = Scatter(
            #id = f'dado_{i}',
            size_hint=(None,None), 
            pos= (pos_x,pos_y),
            ) #este self.dado tiene que tener algún id
            dado_inicial = pos_x + random.randint(200, 200) 
            self.dado.add_widget(image)
            self.espacio_dados.add_widget(self.dado)
...
class CambiarScreen(Screen):
    ...
    def mostrar_dados(self, _):
        ...
        for i in self.Lanzada:
            ...
            image = Image(source= f'sprites/dado_{i}_des.png',)
            dado = Scatter(
                size_hint=(None,None), 
                pos= (random.randint(100,450) , random.randint(200,450)),
            )
            self.ids_y_valores[key] = i
            self.botones_dict[key] = dado
            self.botones_dict[key].add_widget(image)
            self.botones_dict[key].bind(on_touch_down = lambda *args: self.my_function(i, *args))
    ...
    def devolver_dado(self, instance):
        ...
        key = 'dado_' + str(self.counter)
        image = Image(source= f'sprites/dado_{valor_dado}_des.png',)
        dado = Scatter(
            size_hint=(None,None), 
            pos= (random.randint(100,450) , random.randint(200,450)),
            #on_touch_down = lambda *args: self.my_function(str(i), *args)
        )
        self.ids_y_valores[key] = valor_dado
        self.botones_dict[key] = dado
        self.botones_dict[key].add_widget(image)
        #este valor se aprieta cada vez que se devuelve el dado
        self.botones_dict[key].bind(on_touch_down = lambda *args: self.my_function(valor_dado, *args))
        self.ids[key] = self.botones_dict[key]
        self.espacio_dados.add_widget(self.botones_dict[key])
        self.counter += 1
    ...
    def relanzar(self):
        ...
        print(f'La lista de seleccionados es {self.numeros_dados_seleccionados}')
        lista_index_cambiar = []
...
class OpcionesScreen(Screen):
    ...
    def mostrar_opciones(self, _):
        ...
        for k,v in opciones.items():
            ...
            #Boton.bind(on_press=self.opcion_elegida)
        ...
        #Checkear si no volvió a lanzar
